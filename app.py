...
if writer is not None:
	writer.release()
"""
#fetching list of all the students in the class
my_list = os.listdir('mydatabase')
...
def handle_request():
    imagefile = flask.request.files.getlist('image')
    ...
    """
    presentstudents=[]
    for fily in imagefile:
    	response = fily.read()
    	frame = cv2.imdecode(np.fromstring(response, np.uint8), cv2.IMREAD_COLOR)

    	#calling post_process_frame for taking attendence
    	ee=post_process_frame(frame)
    	for w in ee:
    		presentstudents.append(w)

    #removing duplicates
    finalps = list(dict.fromkeys(presentstudents))

    attendance_marker.mark_attendance(finalps)

    #extracting absentees list
    temp=np.array(finalps)
    presenties=temp.tolist()
    res = list((set(my_list)-set(presenties))) 
